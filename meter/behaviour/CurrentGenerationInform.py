...
class CurrentGenerationInform(OneShotBehaviour):
  ...
  async def on_start(self):
    #print("{} - [{}] - Starting CurrentGenerationInform . . .".format(datetime.now(), self.agent.name))
    pass
      ...
      if len(sparqlResult) == 1:
        ...
        if messageData["message_type"] == "CurrentGenerationRequest":
          ...
          if os.path.isfile(filepath):
            ...
            try:
                consumo_atual = await asyncio.to_thread(self.db.obter_geracao_atual)
                #print(f"{datetime.now()} - [{self.agent.name}] - 📊 Current generation: {consumo_atual} kW")
            except Exception as db_error:
                print(f"{datetime.now()} - [{self.agent.name}] - ⚠️ Database error: {db_error}, using default value")
                consumo_atual = 50.0  # Default value

            sparqlConstruct = sparqlTemplate.replace("<[GENERATION]>", str(consumo_atual)).replace("<[MEASUREMENT_DATETIME]>", datetime.now().isoformat()).replace("<[BEGINNING]>", datetime.now().isoformat()).replace("<[END]>", datetime.now().isoformat())
            ...
            response.thread = "current-generation-inform"
  ...
  async def on_end(self):
    #print("{} - [{}] - Ending CurrentGenerationInform . . .".format(datetime.now(), self.agent.name))
    pass