...
class ConsumptionForecastInformProcessor(OneShotBehaviour):
    """
    Processa mensagens ConsumptionForecastInform do Predictor via XMPP direto
    """
    
    def __init__(self,msg=None):
        super().__init__()
        self.message_type = "ConsumptionForecastInform"
        self.thread_filter = "consumption-forecast-inform"
        self.sparql_filename = "get-consumption-forecast-inform-data.sparql"
        self.sparql_cache = {}
    ...
    async def on_start(self):
        #print("{} - [{}] - Starting ConsumptionForecastInformProcessor for thread '{}' . . .".format(
    ...
    async def run(self):
        ...
        try:
            ...
            msg = self.msg # await self.receive(timeout=1.0)
            
            if msg:
                # Verificar se esta mensagem é para este processador
                if self._should_process_message(msg):
                    #print(f"✅ ConsumptionForecastInformProcessor processing message from {msg.sender}")
                    await self.process_message(msg)
                    self.processed_count += 1
                # Se não for para este processador, ignora (outros processadores vão tratar)
                    
        except Exception as e:
            print(f"❌ Error in ConsumptionForecastInformProcessor: {e}")
            traceback.print_exc()
    ...
    def _create_fallback_query(self):
        ...
        SELECT ?data WHERE {
            ?s cao:consumption_forecast ?consumption_forecast .
            ?consumption_forecast cao:hasValue ?data .
        }
        """
    ...
    async def store_data(self, messageData, msg):
        ...
        try:
            ...
            consumption_forecast_value = self._extract_consumption_forecast_value(messageData)

            # Armazenar dados completos no agente
            self.agent.ultima_previsao_consumo = {
                'dados_brutos': messageData,
                'valor_previsao_consumo': consumption_forecast_value,
            ...
            if consumption_forecast_value is not None:
                pass
                #print(f"   📊 Current consumption forecast: {consumption_forecast_value} kW")
            else:
                print(f"   ⚠️ Could not extract numeric consumption forecast value")
            ...
            if hasattr(self.agent, 'mensagens_recebidas'):
                self.agent.mensagens_recebidas.append({
                    'tipo': 'ConsumptionForecastInform',
                    'timestamp': datetime.now().isoformat(),
                    'sender': str(msg.sender),
                    'valor': consumption_forecast_value
                })
            
        except Exception as e:
            print(f"❌ Error storing consumption data: {e}")

    def _extract_consumption_forecast_value(self, messageData):
        """Extrai valor numérico de previsão de consumo dos dados da mensagem"""
        try:
            # Método 1: Estrutura padrão esperada
            if 'consumption_forecast' in messageData:
                consumption_forecast_data = messageData['consumption_forecast']
                if isinstance(consumption_forecast_data, dict):
                    return float(consumption_forecast_data.get('value', 0))
                else:
                    return float(consumption_forecast_data)
            
            # Método 2: Chaves alternativas
            elif 'valor' in messageData:
                return float(messageData['valor'])
            elif 'consumption' in messageData:
                return float(messageData['consumption_forecast'])
            ...
            # Método 3: Dados aninhados em JSON
                ...
                try:
                    ...
                    if 'consumption_forecast' in nested_data:
                        return float(nested_data['consumption_forecast'].get('value', 0))
                except json.JSONDecodeError:
                    pass
            
            # Método 4: Procurar por qualquer chave que contenha 'consumption_forecast'
            for key, value in messageData.items():
                if 'consumption_forecast' in key.lower():
                    try:
            ...
            print(f"⚠️ Consumption Forecast value extraction failed")
            print(f"   Available keys: {list(messageData.keys())}")
            print(f"   Sample data: {dict(list(messageData.items())[:3])}")
            return None
            
        except Exception as e:
            print(f"❌ Error extracting consumption  forecast value: {e}")
            return None

    def _validate_consumption_forecast_data(self, messageData):
        """Valida se os dados de consumo forecast são válidos"""
        try:
            consumption_forecast_value = self._extract_consumption_forecast_value(messageData)

            if consumption_forecast_value is None:
                return False, "No numeric consumption forecast value found"
            
            if consumption_forecast_value < 0:
                return False, f"Negative consumption forecast value: {consumption_forecast_value}"
            
            if consumption_forecast_value > 1000:  # Assumindo limite máximo de 1000 kW
                return False, f"Consumption forecast value too high: {consumption_forecast_value}"
            
            return True, "Valid consumption forecast data"
    ...
    def get_status(self):
        ...
        return {
            'processor_type': 'ConsumptionForecastInformProcessor',
            'message_type': self.message_type,
            'thread_filter': self.thread_filter,
            'processed_count': self.processed_count,
            'last_consumption': getattr(self.agent, 'consumption_forecast', None) is not None
        }

    async def on_end(self):
        #print("{} - [{}] - Ending ConsumptionForecastInformProcessor - Processed {} messages".format(
        #    datetime.now(), self.agent.name, self.processed_count))
        
        # Log final statistics
        if hasattr(self.agent, 'ultima_previsao_consumo') and self.agent.ultima_previsao_consumo:
            consumption_value = self.agent.ultima_previsao_consumo.get('valor_previsao_consumo', '0.0')
            timestamp = self.agent.ultima_previsao_consumo.get('timestamp_recebimento', 'unknown')
            sender = self.agent.ultima_previsao_consumo.get('sender', 'unknown')
            #print(f"   📊 Last consumption forecast value: {consumption_value} kW from {sender} at {timestamp}")
        else:
            pass
            #print(f"   ⚠️ No consumption forecast data received or stored")
        
        # GraphDB update agora é gerenciado pelo MessageServer
        #print(f"� [{self.agent.name}] GraphDB update será gerenciado pelo MessageServer quando dados completos estiverem disponíveis")