...
class ConsumptionForecastRequest(OneShotBehaviour):

  async def on_start(self):
    #print("{} - [{}] - Starting ConsumptionForecastRequest . . .".format(datetime.now(), self.agent.name))
    pass
  
  async def run(self):
    # Get sparql construct file
    filepath = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "sparql", "construct", "consumption-forecast-request.sparql"))
    ...
    if os.path.isfile(filepath):
      ...
      msg.thread = "consumption-forecast-request"
      msg.body = content
  ...
  async def on_end(self):
    print("{} - [{}] - Ending ConsumptionForecastRequest . . .".format(datetime.now(), self.agent.name))