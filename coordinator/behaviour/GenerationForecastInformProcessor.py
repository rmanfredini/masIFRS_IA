...
class GenerationForecastInformProcessor(OneShotBehaviour):
    """
    Processa mensagens GenerationForecastInform do Prediction via XMPP direto
    """

    def __init__(self,msg=None):
        super().__init__()
        self.message_type = "GenerationForecastInform"
        self.thread_filter = "generation-forecast-inform"
        self.sparql_filename = "get-generation-forecast-inform-data.sparql"
        self.sparql_cache = {}
        self.processed_count = 0
        self.msg=msg

    async def on_start(self):
        #print("{} - [{}] - Starting GenerationForecastInformProcessor for thread '{}' . . .".format(
        ...
        try:
            ...
            if msg:
                ...
                if self._should_process_message(msg):
                    #print(f"✅ GenerationForecastInformProcessor processing message from {msg.sender}")
                    await self.process_message(msg)
                    self.processed_count += 1
                # Se não for para este processador, ignora (outros processadores vão tratar)
                    
        except Exception as e:
            print(f"❌ Error in GenerationForecastInformProcessor: {e}")
            traceback.print_exc()
    ...
    def _create_fallback_query(self):
        ...
        SELECT ?data WHERE {
            ?s cao:generation_forecast ?generation_forecast .
            ?generation_forecast cao:hasValue ?data .
        }
        """
    ...
    async def store_data(self, messageData, msg):
        ...
        try:
            # Extrair valor numérico da geração
            generation_forecast_value = self._extract_generation_forecast_value(messageData)

            # Armazenar dados completos no agente
            self.agent.previsao_geracao = {  # ← CORRIGIDO
                'dados_brutos': messageData,
                'valor_previsao_geracao': generation_forecast_value,  # ← CORRIGIDO
            ...
            # Log detalhado
            #print(f"💾 generation forecast data stored from {msg.sender}")
            if generation_forecast_value is None:
                print(f"   ⚠️ Could not extract numeric generation forecast value")
            ...
            if hasattr(self.agent, 'mensagens_recebidas'):
                self.agent.mensagens_recebidas.append({
                    'tipo': 'GenerationForecastInform',
                    'timestamp': datetime.now().isoformat(),
                    'sender': str(msg.sender),
                    'valor': generation_forecast_value
                })
            
        except Exception as e:
            print(f"❌ Error storing generation data: {e}")

    def _extract_generation_forecast_value(self, messageData):
        """Extrai valor numérico de previsão de geracao dos dados da mensagem"""
        try:
            # Método 1: Estrutura padrão esperada
            if 'generation_forecast_value' in messageData:
                generation_forecast_value = messageData['generation_forecast_value']
                if isinstance(generation_forecast_value, dict):
                    return float(generation_forecast_value.get('value', 0))
                else:
                    return float(generation_forecast_value)

            # Método 2: Chaves alternativas
            elif 'valor' in messageData:
                return float(messageData['valor'])
            elif 'generation' in messageData:
                return float(messageData['generation_forecast'])
            ...
            # Método 3: Dados aninhados em JSON
                ...
                try:
                    ...
                    if 'generation_forecast' in nested_data:
                        return float(nested_data['generation_forecast'].get('value', 0))
                except json.JSONDecodeError:
                    pass
            
            # Método 4: Procurar por qualquer chave que contenha 'generation_forecast'
            for key, value in messageData.items():
                if 'generation_forecast' in key.lower():
                    try:
            ...
            print(f"⚠️ generation Forecast value extraction failed")
            print(f"   Available keys: {list(messageData.keys())}")
            print(f"   Sample data: {dict(list(messageData.items())[:3])}")
            return None
            
        except Exception as e:
            print(f"❌ Error extracting generation  forecast value: {e}")
            return None

    def _validate_generation_forecast_data(self, messageData):
        """Valida se os dados de geracao forecast são válidos"""
        try:
            generation_forecast_value = self._extract_generation_forecast_value(messageData)

            if generation_forecast_value is None:
                return False, "No numeric generation forecast value found"
            
            if generation_forecast_value < 0:
                return False, f"Negative generation forecast value: {generation_forecast_value}"
            
            if generation_forecast_value > 1000:  # Assumindo limite máximo de 1000 kW
                return False, f"generation forecast value too high: {generation_forecast_value}"
            
            return True, "Valid generation forecast data"
    ...
    def get_status(self):
        ...
        return {
            'processor_type': 'GenerationForecastInformProcessor',
            'message_type': self.message_type,
            'thread_filter': self.thread_filter,
            'processed_count': self.processed_count,
            'last_generation': getattr(self.agent, 'generation_forecast', None) is not None
        }

    async def on_end(self):
        #print("{} - [{}] - Ending generationForecastInformProcessor - Processed {} messages".format(
         #   datetime.now(), self.agent.name, self.processed_count))
        
        # Log final statistics
        if hasattr(self.agent, 'previsao_geracao') and self.agent.previsao_geracao:
            generation_value = self.agent.previsao_geracao.get('valor_previsao_geracao', '0.0')
            timestamp = self.agent.previsao_geracao.get('timestamp_recebimento', 'unknown')
            sender = self.agent.previsao_geracao.get('sender', 'unknown')
            #print(f"   📊 Last generation forecast value: {generation_value} kW from {sender} at {timestamp}")
        else:
            pass
            #print(f"   ⚠️ No generation forecast data received or stored")
