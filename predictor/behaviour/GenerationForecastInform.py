...
class GenerationForecastInform(OneShotBehaviour):
  ...
  async def on_start(self):
    #print("{} - [{}] - Starting GenerationForecastInform . . .".format(datetime.now(), self.agent.name))
    pass


  async def run(self):
    # Get sparql select file 
    filepath = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "sparql", "select", "get-generation-forecast-request-data.sparql"))
    ...
    if os.path.isfile(filepath):
      ...
      if len(sparqlResult) == 1:
        ...
        if messageData["message_type"] == "GenerationForecastRequest":
          # Get sparql construct file
          filepath = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "sparql", "construct", "generation-forecast-inform.sparql"))
          # DEBUG: print(filepath)
          ...
          if os.path.isfile(filepath):
            ...
            sparqlConstruct = sparqlTemplate.replace("<[GENERATION_FORECAST]>", str(self.prever_geracao())).replace("<[MEASUREMENT_DATETIME]>", datetime.now().isoformat()).replace("<[BEGINNING]>", datetime.now().isoformat()).replace("<[END]>", datetime.now().isoformat())
            ...
            response.thread = "generation-forecast-inform"
  ...
  async def on_end(self):
    #print("{} - [{}] - Ending GenerationForecastInform . . .".format(datetime.now(), self.agent.name))
    pass
    
  def prever_geracao(self): 
        """
        Previsão de geração para as próximas horas.
        """
        #print(f'prever_geracao: {self.campus}')
        # Criar instância do forecaster e executar previsão
        forecaster = ChronosForecaster(mode='geracao', campus_id=self.campus, forecast_horizon=1, plot_range=96, plotar=False)
        previsoes_df, erro = forecaster.run_forecast(save_plot=False)
        
        # Extrair apenas o valor mediano da primeira (e única) previsão
        valor_previsto = float(previsoes_df['median'].iloc[0])
        
        #print(f'geracao prevista: {valor_previsto}')
